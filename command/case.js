...
sendButLocation(from, lang.mzstore(prefix, pushname), '© ' + ownername, mzstore, [{buttonId: '.mllist', buttonText: {displayText: 'Topup ml'}, type: 1},{buttonId: '.jokilist', buttonText:{displayText: 'Jasa joki'}, type: 1},{buttonId: '.roomlist', buttonText:{displayText: 'Room turn'}, type: 1}], {quoted: mek})
				...
				break
case 'mllist':{
	var po = await getBuffer(`https://i.ibb.co/tM0QffF/IMG-20220329-WA0230.jpg`)
    haruka.sendMessage(from, po, image, { quoted: mek })
    }
    break
case 'jokilist':
...
case 'roomlist':{
    ...
    haruka.sendMessage(from, po, image, {thumbnail: Buffer.alloc(0)}, { quoted: mek })
    }
    break
case 'infobot':
...
case 'owner':{
	if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk dibawah`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
		const ownerContact = [ownernumber,6282274429758]
		let ini_list = []
...
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk dibawah`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
    if (args.length == 0) return reply(`Example: ${prefix + command} Rifai`)
    ini_txt = args.join(" ")
...
case 'memegenerator': case 'memegen':{
						...
						if (args.length < 1) return reply(`Kirim perintah *${prefix + command}* RifaiBot`)
									if (q.includes('|')) return reply(`Kirim perintah *${prefix + command}* RifaiBot`)
									try {
										if (!isQuotedImage) return reply(`Reply Gambar!`)
										reply(lang.wait())
										var teks2 = args.join(' ')
...
case 'nuliskiri':{
									...
									.on('exit', () => {
										haruka.sendMessage(from, fs.readFileSync('./database/media/nulis/images/buku/setelahkiri.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `Sip`})
						...
						case 'nuliskanan':{
									...
									.on('error', () => reply(mess.error))
									.on('exit', () => {
										haruka.sendMessage(from, fs.readFileSync('./database/media/nulis/images/buku/setelahkanan.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `Sip`})
						...
						case 'foliokiri':{
									...
									.on('error', () => reply(mess.error))
									.on('exit', () => {
										haruka.sendMessage(from, fs.readFileSync('./database/media/nulis/images/folio/setelahkiri.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `Sip`})
						...
						case 'foliokanan':{
									...
									.on('error', () => reply(mess.error))
									.on('exit', () => {
										haruka.sendMessage(from, fs.readFileSync('./database/media/nulis/images/folio/setelahkanan.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `Sip`})
